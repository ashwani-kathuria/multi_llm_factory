...
class BaseLLMWrapper(abc.ABC):
    ...
    def _log_metrics(
        self, start_time: float, prompt_tokens: int, completion_tokens: int
    ) -> LLMExecutionMetrics:
        ...
        logger.info(
            f"Target '{self.config.model_id}' query resolved. Time: {metrics.latency_ms}ms | Total Tokens: {metrics.total_tokens}"
        )
        return metrics
...
class OpenAIWrapper(BaseLLMWrapper):
    ...
    def generate(
        ...
        metrics = self._log_metrics(
            start_time, response.usage.prompt_tokens, response.usage.completion_tokens
        )
        return LLMUnifiedResponse(
            content=choice.content or "",
            structured_json=(
                choice.parsed.model_dump()
                if response_schema and choice.parsed
                else None
            ),
            metrics=metrics,
        )
...
class GoogleGenAIWrapper(BaseLLMWrapper):
    ...
    def _build_config(
        self, system_instruction: Optional[str], response_schema: Optional[Type[Any]]
    ) -> Any:
    ...
    def generate(
        ...
        struct_data = (
            json.loads(response.text) if response_schema and response.text else None
        )
        return LLMUnifiedResponse(
            content=response.text or "", structured_json=struct_data, metrics=metrics
        )
...
class AWSBedrockWrapper(BaseLLMWrapper):
    def _initialize_client(self) -> Any:
        ...
        return boto3.client(
            "bedrock-runtime", region_name=self.config.aws_region, config=botocore_cfg
        )
    ...
    def _prepare_payload(
        ...
        if response_schema:
            payload["additionalModelRequestFields"] = {
                "outputConfig": {
                    "textFormat": {"json": response_schema.model_json_schema()}
                }
            }
        return payload
    ...
    def generate(
        ...
        metrics = self._log_metrics(
            start_time, usage.get("inputTokens", 0), usage.get("outputTokens", 0)
        )
        struct_data = json.loads(output_text) if response_schema else None
        return LLMUnifiedResponse(
            content=output_text, structured_json=struct_data, metrics=metrics
        )
    ...
    async def agenerate(
        ...
        return await loop.run_in_executor(
            None, self.generate, prompt, system_instruction, response_schema
        )


# ==========================================
# OLLAMA STRATEGY DRIVER
# ==========================================
class OllamaLLMWrapper(BaseLLMWrapper):
    ...
    def generate(
        ...
        response = self.client["sync"].chat(
            model=self.config.model_id,
            messages=messages,
        ...
        p_tok = response.get("prompt_eval_count", 0)
        c_tok = response.get("eval_count", 0)
        metrics = self._log_metrics(start_time, p_tok, c_tok)

        struct_data = json.loads(content) if response_schema and content else None
        return LLMUnifiedResponse(
            content=content, structured_json=struct_data, metrics=metrics
        )
    ...
    async def agenerate(
        ...
        response = await self.client["async"].chat(
            model=self.config.model_id,
            messages=messages,
            format="json" if response_schema else None,
            options={
