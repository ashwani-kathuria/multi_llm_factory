...
def parse_command_line_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["openai", "google", "aws_bedrock", "ollama"],
...
async def main():
    ...
    try:
        ...
        if args.prompt_file:
            if not os.path.exists(args.prompt_file):
                raise FileNotFoundError(
                    f"The specified prompt file target was not found: '{args.prompt_file}'"
                )
            ...
            if not prompt_text:
                raise ValueError(
                    f"The prompt file '{args.prompt_file}' is completely empty."
                )
        else:
        ...
        config = LLMModelConfig(
            provider=args.provider, model_id=args.model, temperature=args.temperature
        )
        ...
        logging.error(
            f"A catastrophic pipeline execution failure occurred: {runtime_err}"
        )
